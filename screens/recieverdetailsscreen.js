...
export default class Recieverinfo extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            bookname:this.props.navigtaion.getParam('details')["bookname"],
            reason:this.props.navigaion.getParam('details')["reason"],
            recievername:'',
            recievercontact:'',
            recieveraddress:'',
    ...
    getrecievrdetail(){
        ...
        db.collection('requestbooks').where('requestid','===',this.state.requestid).get().then(snapshot=>{
            snapshot.forEach(doc=>{
                this.setState({recieverrequestdocid:docid})
            })
        })
    }
    updatebookstatus = ()=>{
